  ...
  methods: {
    ...
    handleImageAdded: function(file, Editor, cursorLocation, resetUploader) {
      ...
      this.$axios({
        ...
        .then(res => {
          let url = this.$fixImgUrl(res.url); // Get url from response
    ...
    handleSuccess(res) {
      console.log(res);
      // this.form.cover.push(res)

      this.form.cover.push({
        uid: res.id,
        url: this.$fixImgUrl(res.url)
      });
    },
    onSubmit() {
      var submitNo = 0;
      ...
      this.$router.push("/message/postlist");
    }
  }
};
</script>
