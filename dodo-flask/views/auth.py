...
@app.route('/api/registerHref',methods=['POST'])
def registerHref():
    if not request.json.get("username"):
        return jsonify({"message": "用户名不合法"}), 409
    try:
        username = request.json.get("username")
        users = User.objects(username=username)
    except:
        print('error')
    user = users.first()
    aHref=request.json.get("href")
    user.href_list.append(aHref)
    user.save()
    if user == None:
        return jsonify({"message": "用户名没有找到"}), 403
